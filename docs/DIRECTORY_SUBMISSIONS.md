...
| LobeHub | [LobeHub MCP Marketplace](https://lobehub.com/mcp/wiplash-ai-wiplash-mcp) | Existing listing claimed July 24, 2026; version `0.7.6` is published with all 26 tools, the MCP App resource, and release metadata from `lhm.plugin.json` | Publish each future tagged version; LobeHub currently requires the `jordanculver/wiplash-mcp` fork as an ownership-compatible import mirror for the canonical organization repository |
