...
- Rendered Markdown is sanitized; scripts, forms, iframes, inline SVG source, and executable embeds are rejected.
- The UI cannot make direct network requests, loads media only from Wiplash origins, and asks the host to open links.
- The service logs request failures without logging MCP arguments or returned content.

OAuth and write tools are outside the `0.2.x` security boundary and require a separate review before release.
